...
智能体行也可以选择使用 `branch` 和 `worktree`,它们描述的是该窗格自身的 Git 检出,而不是智能体自身。两者都按窗格解析,因此同一工作区中运行在不同 worktree 里的智能体会显示各自的分支。各自渲染什么、何时省略,请参见 [Configuration](/docs/zh-cn/configuration/) 中的智能体行 token 列表。
