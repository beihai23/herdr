...
Agent rows can also opt into `branch`, `worktree`, and `repo`, which describe the pane's own Git checkout rather than the agent itself. All three are resolved per pane, so agents in one workspace that run in separate worktrees report their own branches while agreeing on the repository. See the Agent row token list under [Configuration](/docs/configuration/) for what each one renders and when it is omitted.
