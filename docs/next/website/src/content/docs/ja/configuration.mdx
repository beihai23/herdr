...
- `branch` — 利用可能な場合の、そのペイン自身のチェックアウトの Git ブランチ。ペインごとにその作業ディレクトリから解決するため、同じワークスペース内で別々の worktree で動くペインはそれぞれのブランチを表示します。
- `worktree` — ペインがリンクされた Git worktree のチェックアウトで動いているときは `worktree`、それ以外では省略。ディスク上のチェックアウトから検出するため、Herdr の外で `git worktree add` により作成された worktree もマークされます。メインのチェックアウトのエージェントにはマークが付かず、メインのチェックアウトであるかのようには表示しません。判断の手がかりにはマークの「有ること」を使ってください。
- `repo` — そのペインのチェックアウトが属するリポジトリ名。Git の外では省略。`branch` と同様にペインごとに解決するため、メインのチェックアウトとそのリンクされた worktree はすべて同じリポジトリを報告しつつ、それぞれ自分のブランチを報告します。1 つのリポジトリの複数の worktree を同時に開いているときに使ってください。ワークスペースのラベルは worktree の名前（Herdr がブランチから導出）を示すため、リポジトリ自体を識別するのは `repo` です。
